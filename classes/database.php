...
class Database{
  ...
  public function sub_data($kec){
    $query = "SELECT No as id,Kelurahan,Kecamatan,P,L,Jumlah FROM penduduk WHERE Kecamatan = '$kec'";
    $sql = $this->mysqli->query($query);
    return $sql;
  }

  public function edit($id){
    $query = "SELECT No,Kelurahan,Kecamatan,P,L FROM penduduk WHERE No = '$id'";
    $sql = $this->mysqli->query($query);
    return $sql;
  }
